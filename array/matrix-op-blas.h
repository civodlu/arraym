...
using Matrix_BlasEnabled = typename std::enable_if<array_use_blas<Array<T, N, Config>>::value && is_matrix<Array<T, N, Config>>::value, Array<T, N, Config>>::type;
...
namespace details
{
   ...
   Matrix_BlasEnabled<T, 2, Config> array_mul_array(const Array<T, 2, Config>& opa, const Array<T, 2, Config2>& opb)
   {
      ...
      core::blas::gemm<T>(order,
         blas::CblasNoTrans, blas::CblasNoTrans, (blas::BlasInt)opa.rows(), (blas::BlasInt)opb.columns(), (blas::BlasInt)opa.columns(),
         (T)1.0,
         &opa(0, 0), lda,
         &opb(0, 0), ldb,
         (T)0.0,
