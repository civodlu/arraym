...
#define CHECK_CUDA(expr) ensure((expr) == CUBLAS_STATUS_SUCCESS, "CUBLAS failed!");

#define INIT_AND_CHECK_CUDA(expr)                                                                                                                              \
   {                                                                                                                                                           \
      NAMESPACE_NLL::blas::detail::config.init();                                                                                                              \
      ensure((expr) == CUBLAS_STATUS_SUCCESS, "CUBLAS failed!");                                                                                               \
   }

// do a CUDA function call. Always make sure the init was run beforehand
#define CUDA_PASSED(expr) NAMESPACE_NLL::blas::detail::config.init(), (!((expr) == CUBLAS_STATUS_SUCCESS));
