   ...
   namespace details
   {
      ...
      */
      ...
      public:
         ...
         typedef std::function < void( CBLAS_ORDER matrixOrder,
         ...
         typedef std::function < void( CBLAS_ORDER matrixOrder,
                                       ...
                                       const BlasReal *y, const BlasInt incY, BlasReal *A, const BlasInt lda ) > sger_t;
         typedef std::function < void( CBLAS_ORDER matrixOrder,
                                       const BlasInt M, const BlasInt N, const BlasDoubleReal alpha, const BlasDoubleReal *x, const BlasInt incX,
                                       const BlasDoubleReal *y, const BlasInt incY, BlasDoubleReal *A, const BlasInt lda ) > dger_t;
         ...
         typedef std::function < void( CBLAS_ORDER matrixOrder,
                                       ...
                                       const BlasReal beta, BlasReal *y, const BlasInt incY ) > sgemv_t;
         typedef std::function < void( CBLAS_ORDER matrixOrder,
                                       const enum CBLAS_TRANSPOSE TransA, const BlasInt M, const BlasInt N,
                                       const BlasDoubleReal alpha, const BlasDoubleReal *A, const BlasInt lda, const BlasDoubleReal *x, const BlasInt incX,
                                       const BlasDoubleReal beta, BlasDoubleReal *y, const BlasInt incY ) > dgemv_t;
         ...
         typedef std::function<BlasInt( CBLAS_ORDER matrixOrder, char jobz, BlasInt m,
                                        BlasInt n, BlasReal* a, BlasInt lda, BlasReal* s,
                                        BlasReal* u, BlasInt ldu, BlasReal* vt,
                                        BlasInt ldvt )> sgesdd_t;
         typedef std::function<BlasInt( CBLAS_ORDER matrixOrder, char jobz, BlasInt m,
                                        BlasInt n, BlasDoubleReal* a, BlasInt lda, BlasDoubleReal* s,
                                        BlasDoubleReal* u, BlasInt ldu, BlasDoubleReal* vt,
                                        BlasInt ldvt )> dgesdd_t;

         typedef std::function < BlasInt( CBLAS_ORDER matrixOrder, BlasInt n, BlasReal* a,
                                          BlasInt lda, BlasInt k1, BlasInt k2,
                                          const BlasInt* ipiv, BlasInt incx ) > slaswp_t;
         typedef std::function < BlasInt( CBLAS_ORDER matrixOrder, BlasInt n, BlasDoubleReal* a,
                                          BlasInt lda, BlasInt k1, BlasInt k2,
                                          const BlasInt* ipiv, BlasInt incx ) > dlaswp_t;

         typedef std::function < BlasInt( CBLAS_ORDER matrix_order, char trans, BlasInt n,
                                          BlasInt nrhs, const BlasReal* a, BlasInt lda,
                                          const BlasInt* ipiv, BlasReal* b, BlasInt ldb ) > sgetrs_t;

         typedef std::function < BlasInt( CBLAS_ORDER matrix_order, char trans, BlasInt n,
                                          BlasInt nrhs, const BlasDoubleReal* a, BlasInt lda,
                                          const BlasInt* ipiv, BlasDoubleReal* b, BlasInt ldb ) > dgetrs_t;


         typedef std::function < BlasInt( CBLAS_ORDER matrix_order, char trans, BlasInt m, BlasInt n,
                                          BlasInt nrhs, BlasReal* a, BlasInt lda, BlasReal* b, BlasInt ldb ) > sgels_t;
         typedef std::function < BlasInt( CBLAS_ORDER matrix_order, char trans, BlasInt m, BlasInt n,
                                          BlasInt nrhs, BlasDoubleReal* a, BlasInt lda, BlasDoubleReal* b, BlasInt ldb ) > dgels_t;


      private:
         typedef std::tuple < std::vector<saxpy_t>,
            std::vector<daxpy_t>,
            std::vector<sasum_t>,
            std::vector<dasum_t>,
            std::vector<sdot_t>,
