   ...
   namespace detail
   {
      ...
      */
         ...
         const BlasReal beta, BlasReal* C, const BlasInt ldc)
      {
         ...
         return CUDA_PASSED(cublasXtSgemm(config.handle(), is_transposed(matrixOrder, TransA), is_transposed(matrixOrder, TransB), M, N, K, &alpha, A, lda, B, ldb, &beta, C, ldc));
      }
         ...
         const BlasDoubleReal beta, BlasDoubleReal* C, const BlasInt ldc)
      {
         ...
         return CUDA_PASSED(cublasXtDgemm(config.handle(), is_transposed(matrixOrder, TransA), is_transposed(matrixOrder, TransB), M, N, K, &alpha, A, lda, B, ldb, &beta, C, ldc));
      }
   }
}
