...
BlasInt gemm_cublas(CBLAS_ORDER matrixOrder, const enum CBLAS_TRANSPOSE TransA, const enum CBLAS_TRANSPOSE TransB, const BlasInt M, const BlasInt N,
                    const BlasInt K, const T alpha, const T* A, const BlasInt lda, const T* B, const BlasInt ldb, const T beta, T* C, const BlasInt ldc, F f)
{
   using value_type = T;
   ...
   cuda::gpu_ptr_type<value_type> gpu_ptr_a =
       (TransA == CBLAS_TRANSPOSE::CblasNoTrans) ? cuda::matrix_cpu_to_gpu(M, K, A, lda) : cuda::matrix_cpu_to_gpu(K, M, A, lda);
   cuda::gpu_ptr_type<value_type> gpu_ptr_b =
       (TransB == CBLAS_TRANSPOSE::CblasNoTrans) ? cuda::matrix_cpu_to_gpu(K, N, B, ldb) : cuda::matrix_cpu_to_gpu(N, K, B, ldb);
   ...
   INIT_AND_CHECK_CUDA(
       f(config.handle(), transa, transb, M, N, K, &alpha, gpu_ptr_a.get(), gpu_stride_a, gpu_ptr_b.get(), gpu_stride_b, &beta, gpu_ptr_c.get(), ld_gpu));
...
{
   return gemm_cublas<BlasReal>(matrixOrder, TransA, TransB, M, N, K, alpha, A, lda, B, ldb, beta, C, ldc, &cublasSgemm);
}
...
{
   return gemm_cublas<BlasDoubleReal>(matrixOrder, TransA, TransB, M, N, K, alpha, A, lda, B, ldb, beta, C, ldc, &cublasDgemm);
}
...
BlasInt saxpy_cublas(BlasInt N, BlasReal alpha, const BlasReal* x, BlasInt incx, BlasReal* y, BlasInt incy)
{
   ...
   INIT_AND_CHECK_CUDA(cublasSaxpy(config.handle(), N, &alpha, gpu_ptr_x.get(), 1, gpu_ptr_y.get(), 1));
...
BlasInt daxpy_cublas(BlasInt N, BlasDoubleReal alpha, const BlasDoubleReal* x, BlasInt incx, BlasDoubleReal* y, BlasInt incy)
{
   ...
   INIT_AND_CHECK_CUDA(cublasDaxpy(config.handle(), N, &alpha, gpu_ptr_x.get(), 1, gpu_ptr_y.get(), 1));
...
BlasReal sdot_cublas(const BlasInt N, const BlasReal* x, const BlasInt incX, const BlasReal* y, const BlasInt incY)
{
   ...
   BlasReal result = 0;
   INIT_AND_CHECK_CUDA(cublasSdot(config.handle(), N, gpu_ptr_x.get(), 1, gpu_ptr_y.get(), 1, &result));
   return result;
}
...
BlasDoubleReal ddot_cublas(const BlasInt N, const BlasDoubleReal* x, const BlasInt incX, const BlasDoubleReal* y, const BlasInt incY)
{
   ...
   INIT_AND_CHECK_CUDA(cublasDdot(config.handle(), N, gpu_ptr_x.get(), 1, gpu_ptr_y.get(), 1, &result));
   return result;
}
...
BlasReal snrm2_cublas(const BlasInt N, const BlasReal* X, const BlasInt incX)
{
   auto gpu_ptr_x  = cuda::vector_cpu_to_gpu(N, X, incX);
   BlasReal result = 0;
   INIT_AND_CHECK_CUDA(cublasSnrm2(config.handle(), N, gpu_ptr_x.get(), 1, &result));
   return result;
}
...
BlasDoubleReal dnrm2_cublas(const BlasInt N, const BlasDoubleReal* X, const BlasInt incX)
{
   auto gpu_ptr_x        = cuda::vector_cpu_to_gpu(N, X, incX);
   BlasDoubleReal result = 0;
   INIT_AND_CHECK_CUDA(cublasDnrm2(config.handle(), N, gpu_ptr_x.get(), 1, &result));
   return result;
}
...
BlasReal sasum_cublas(const BlasInt n, const BlasReal* x, const BlasInt incx)
{
   auto gpu_ptr_x  = cuda::vector_cpu_to_gpu(n, x, incx);
   BlasReal result = 0;
   INIT_AND_CHECK_CUDA(cublasSasum(config.handle(), n, gpu_ptr_x.get(), 1, &result));
   return result;
}
...
BlasDoubleReal dasum_cublas(const BlasInt n, const BlasDoubleReal* x, const BlasInt incx)
{
   auto gpu_ptr_x        = cuda::vector_cpu_to_gpu(n, x, incx);
   BlasDoubleReal result = 0;
   INIT_AND_CHECK_CUDA(cublasDasum(config.handle(), n, gpu_ptr_x.get(), 1, &result));
   return result;
}

template <class T, class F>
BlasInt ger_cublas(CBLAS_ORDER matrixOrder, const BlasInt M, const BlasInt N, const T alpha, const T* x, const BlasInt incX, const T* y, const BlasInt incY,
                   T* A, const BlasInt lda, F f)
{
   using value_type = T;
...
BlasInt sger_cublas(CBLAS_ORDER matrixOrder, const BlasInt M, const BlasInt N, const BlasReal alpha, const BlasReal* x, const BlasInt incX, const BlasReal* y,
